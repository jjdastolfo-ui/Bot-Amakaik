...
db.exec(`
  ...
  CREATE TABLE IF NOT EXISTS stock_ganadero (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    campo TEXT DEFAULT 'LA ESCONDIDA',
...
try { db.exec(`INSERT OR IGNORE INTO campos (nombre, orden) VALUES ('LA ESCONDIDA', 0)`); } catch(e) {}
try { db.exec(`ALTER TABLE lotes ADD COLUMN campo TEXT DEFAULT 'LA ESCONDIDA'`); } catch(e) {}
try { db.exec(`ALTER TABLE ordenes_trabajo ADD COLUMN campo TEXT DEFAULT 'LA ESCONDIDA'`); } catch(e) {}
try { db.exec(`ALTER TABLE stock_productos ADD COLUMN campo TEXT DEFAULT 'LA ESCONDIDA'`); } catch(e) {}
try { db.exec(`ALTER TABLE bienes_muebles ADD COLUMN campo TEXT DEFAULT 'LA ESCONDIDA'`); } catch(e) {}
// Rellenar los registros viejos (por si la columna quedó en NULL)
try { db.exec(`UPDATE lotes SET campo='LA ESCONDIDA' WHERE campo IS NULL OR campo=''`); } catch(e) {}
try { db.exec(`UPDATE ordenes_trabajo SET campo='LA ESCONDIDA' WHERE campo IS NULL OR campo=''`); } catch(e) {}
try { db.exec(`UPDATE stock_productos SET campo='LA ESCONDIDA' WHERE campo IS NULL OR campo=''`); } catch(e) {}
try { db.exec(`UPDATE bienes_muebles SET campo='LA ESCONDIDA' WHERE campo IS NULL OR campo=''`); } catch(e) {}
try { db.exec(`UPDATE stock_ganadero SET campo='LA ESCONDIDA' WHERE campo IS NULL OR campo=''`); } catch(e) {}
...
function comprarInsumo(o) {
  const campo = (o.campo || 'LA ESCONDIDA').toUpperCase();
...
app.post("/api/bienes", (req, res) => {
  ...
  const r = db.prepare(`INSERT INTO bienes_muebles (nombre, categoria, valor_compra, fecha_compra, vida_util_anios, valor_residual, notas, campo)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)`).run(nombre, categoria || '', parseFloat(valor_compra), fecha_compra, parseFloat(vida_util_anios) || 10, parseFloat(valor_residual) || 0, notas || '', (campo||'LA ESCONDIDA').toUpperCase());
...
app.post("/api/diario", (req, res) => {
  ...
  const r = db.prepare("INSERT INTO diario_campo (campo, fecha, tipo, mm, titulo, detalle) VALUES (?, ?, ?, ?, ?, ?)")
    .run((campo || 'LA ESCONDIDA').toUpperCase(), fecha, t, (mm != null && mm !== '') ? parseFloat(mm) : null, titulo || '', detalle || '');
...
app.post("/api/ganado", (req, res) => {
  ...
  const r = db.prepare(`INSERT INTO stock_ganadero (campo, categoria, registro, cantidad, cantidad_venta, kg_estimado, valor_cabeza, orden, notas)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(campo || 'LA ESCONDIDA', categoria, (registro || 'GENERAL').toUpperCase(),
...
app.post("/api/ganado/sync-ade", async (req, res) => {
  ...
  try {
    ...
    const ins = db.prepare("INSERT INTO stock_ganadero (campo, categoria, registro, cantidad, cantidad_venta, kg_estimado, valor_cabeza, origen) VALUES ('LA ESCONDIDA', ?, ?, ?, ?, 0, 0, 'ade')");
...
app.post("/api/lotes", (req, res) => {
  ...
  try {
    const r = db.prepare("INSERT INTO lotes (nombre,hectareas,ha_sembrables,notas,campo) VALUES (?,?,?,?,?)").run(nombre.toUpperCase(), hectareas||0, ha_sembrables||0, notas||'', (campo||'LA ESCONDIDA').toUpperCase());
...
app.post("/api/stock", (req, res) => {
  ...
  try {
    const r = db.prepare("INSERT INTO stock_productos (nombre,rubro,categoria,unidad,cantidad,precio_unitario,notas,campo) VALUES (?,?,?,?,?,?,?,?)")
      .run(nombre.toUpperCase(), (rubro||'AGRICOLA').toUpperCase(), (categoria||'OTRO').toUpperCase(), unidad||'kg', parseFloat(cantidad)||0, parseFloat(precio_unitario)||0, notas||'', (campo||'LA ESCONDIDA').toUpperCase());
...
app.post("/api/ordenes", (req, res) => {
  ...
  const r = db.prepare("INSERT INTO ordenes_trabajo (numero,anio,lote,titulo,ciclo,hectareas,estado,notas,campo) VALUES (?,?,?,?,?,?,'PLANIFICADA',?,?)")
    .run(numFinal, anioFinal, loteStr, titulo||'', ciclo||'', ha, notas||'', (req.body.campo||'LA ESCONDIDA').toUpperCase());
...
app.get("/", (req, res) => res.json({ status: "AMAKAIK Bot activo 🟢", version: "1.0.0-amakaik" }));

const PORT = process.env.PORT || 3000;
app.listen(PORT, () => {
  console.log(`AMAKAIK Bot v1.0.0-amakaik corriendo en puerto ${PORT}`);
